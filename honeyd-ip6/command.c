...
void cmd_trigger_read(struct command *cmd, int size)
{
...
void cmd_trigger_write(struct command *cmd, int size)
{
	if (cmd->pfd == -1 || !cmd->fdconnected)
		return;
	if (size)
...
void cmd_ready_fd(struct command *cmd, struct callback *cb, void *con)
{
	TRACE(cmd->pfd,
	...
	{
		TRACE(cmd->perrfd,
				event_set(&cmd->peread, cmd->perrfd, EV_READ, cb->cb_eread, con));
	}
}
...
void cmd_environment(struct template *tmpl, struct tuple *hdr)
{
	char line[256];
	struct addr addr;
	struct ip_hdr ip;
	...
	/* set env. variables (src dst ip etc) for the scripts */
	if (hdr->src_addr.addr_type == ADDR_TYPE_IP6
			&& hdr->dst_addr.addr_type == ADDR_TYPE_IP6)
	{
		snprintf(line, sizeof(line), "%s", addr_ntoa(&hdr->src_addr));
		setenv("HONEYD_IP_SRC", line, 1);

		snprintf(line, sizeof(line), "%s", addr_ntoa(&hdr->dst_addr));
...
void cmd_droppriv(uid_t uid, gid_t gid)
{
...
int cmd_setpriv(struct template *tmpl)
{
	...
	/* TODO: causes problems in valgrind */
...
int cmd_fork(struct tuple *hdr, struct command *cmd, struct template *tmpl,
		char *execcmd, char **argv, void *con)
{
	extern int honeyd_nchildren;
	/* pair and perr contain the two sockets */
	int pair[2], perr[2];
	struct callback *cb;
	sigset_t sigmask;
...
int cmd_python(struct tuple *hdr, struct command *cmd, void *con)
{
	int pair[2];
	struct callback *cb;
...
int cmd_subsystem(struct template *tmpl, struct subsystem *sub, char *execcmd,
		char **argv)
{
	extern int honeyd_nchildren;
...
static void cmd_subsystem_connect_cb(int fd, short what, void *arg)
{
	...
	out: port_encapsulation_free(tmp);
}
		...
		struct port *port, void *con)
{
	...
	TRACE(port->sub_fd,
			event_set(&tmp->ev, port->sub_fd, EV_WRITE, cmd_subsystem_connect_cb, tmp));
...
int cmd_subsystem_localconnect(struct tuple *hdr, struct command *cmd,
		struct port *port, void *con)
{
	syslog(LOG_DEBUG, "subsystem local connect------------------------------");
	struct callback *cb;
	struct subsystem *sub = port->sub;
