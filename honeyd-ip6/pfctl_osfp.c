...
int pfctl_file_fingerprints(int dev, int opts, const char *fp_filename)
{
	FILE *in;
	char *line;
	size_t len;
	int i, lineno = 0;
	int window, w_mod, ttl, df, psize, p_mod, mss, mss_mod, wscale, wscale_mod,
			optcnt, ts0;
...
#define GET_STR(v, n, mn) \
	get_str(&line, &len, &v, n, mn, fp_filename, lineno)
		...
		if (class[0] == '@' || class[0] == '*')
			...
			strlcpy(fp.fp_os.fp_class_nm, class, sizeof(fp.fp_os.fp_class_nm));
...
void pfctl_show_fingerprints(int opts)
{
...
pf_osfp_t pfctl_get_fingerprint(const char *name)
{
...
#define CONNECTOR(x)	((x) == '.' || (x) == ' ' || (x) == '\t' || (x) == '-')
		...
		LIST_FOREACH(version_nm, &class_nm->nm_sublist, nm_entry)
		{
			...
			if (wr_len < version_len + 2 || !CONNECTOR(ptr[version_len]))
				continue;
					...
					nm_entry)
			{
				...
				if (strcasecmp(&ptr[version_len + 1], subtype_nm->nm_name) != 0)
					continue;
...
void add_fingerprint(int dev, int opts, struct pf_osfp_ioctl *fp)
{
...
#define EXPAND(field) do {						\
	...
	if (nm_version)
	{
		...
		if (nm_subtype)
		{
			if (nm_subtype->nm_num == 0)
				nm_subtype->nm_num = ++nm_version->nm_sublist_num;
...
#endif /* FAKE_PF_KERNEL */
	{
		if (errno == EEXIST)
		{
			warn("Duplicate signature for %s %s %s", fp->fp_os.fp_class_nm,
					fp->fp_os.fp_version_nm, fp->fp_os.fp_subtype_nm);
...
void import_fingerprint(struct pf_osfp_ioctl *fp)
{
	struct name_entry *nm_class, *nm_version, *nm_subtype;
	int class, version, subtype;
...
void print_name_list(int opts, struct name_list *nml, const char *prefix)
{
	char newprefix[32];
	struct name_entry *nm;
	...
	{
		snprintf(newprefix, sizeof(newprefix), "%s%s\t", prefix, nm->nm_name);
		printf("%s\n", newprefix);
		print_name_list(opts, &nm->nm_sublist, newprefix);
	}
}

void sort_name_list(int opts, struct name_list *nml)
{
...
int get_int(char **line, size_t *len, int *var, int *mod, const char *name,
...
int get_str(char **line, size_t *len, char **v, const char *name, int minlen,
...
int get_tcpopts(const char *filename, int lineno, const char *tcpopts,
	...
	for (i = 0; tcpopts[i] && *optcnt < PF_OSFP_MAX_OPTS;)
	{
		...
		case 'S':
			*packed = (*packed << PF_OSFP_TCPOPT_BITS)
					| (opt == 'N' ? PF_OSFP_TCPOPT_NOP : PF_OSFP_TCPOPT_SACK);
			break;
		case 'W': /* FALLTHROUGH */
		case 'M':
		{
			...
			*packed = (*packed << PF_OSFP_TCPOPT_BITS)
					| (opt == 'W' ? PF_OSFP_TCPOPT_WSCALE : PF_OSFP_TCPOPT_MSS);
			if (tcpopts[i] == '*'
		...
		case 'T':
			...
			*packed = (*packed << PF_OSFP_TCPOPT_BITS) | PF_OSFP_TCPOPT_TS;
			break;
		}
		(*optcnt)++;
		if (tcpopts[i] == '\0')
