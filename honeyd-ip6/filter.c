...
static int filter_compare(struct filter *a, struct filter *b)
{
	if (a->count < b->count)
		return (-1);
...
void filter_free(struct filtertree *filters)
{
...
void filter_top(struct filtertree *filters, int n, void (*cb)(void *, void *),
		void *arg)
{
	int i = 0;
	struct filter *filter;
