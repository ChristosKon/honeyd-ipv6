...
pyextend_populate_connections(struct tuple *hdr, void *arg)
{
	...
	if(hdr->src_addr.addr_type == ADDR_TYPE_NONE || hdr->dst_addr.addr_type == ADDR_TYPE_NONE){	
...
pyextend_interfaces(PyObject *self, PyObject *args)
{
	...
	for (i = 0; i < interface_count(); i++) {
		...
		pValue = Py_BuildValue("{sssssssiss}",
		    "name", if_ent->intf_name,
		    "address", addr_ntoa(&if_ent->intf_addr),
		    "address6", addr_ntoa(&inter->if_ent.intf_alias_addrs[0]),
		    "mtu", if_ent->intf_mtu,
