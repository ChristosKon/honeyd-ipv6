...
/* ipv6 (nmap) fingerprint tree */
SPLAY_GENERATE(perstree6, personality, node, perscompare);

/* ET - For the Xprobe fingerprint tree */
SPLAY_GENERATE(xp_fprint_tree, xp_fingerprint, node, xp_fprint_compare);

/* cb gets called every 100000 ms */
static void
...
personality_new(const char *name)
{
	...
	/* return NULL if the personality does already exists */
	tmp.name = (char *)name;
...
/* 
 * Inserts a new ipv6 fingerprint into the tree.
 * Returns NULL if the personality does already exists.
 */
struct personality *
personality_new6(const char *name)
{
	struct personality *pers, tmp;
	/* return NULL if the personality does already exists */
	tmp.name = (char *)name;
	if (SPLAY_FIND(perstree6, &personalities6, &tmp))
		return (NULL);
...
void
personality_init6(void)
{
	SPLAY_INIT(&personalities6);
	/* TODO: do an actual file parsing here */
	personality_new6("Microsoft Windows 7 SP1");
}
...
struct personality *
personality_find6(const char *name)
{
	struct personality tmp;
	tmp.name = (char *)name;
 ...
 * lineno contains the number of the line where the fingerprint begins
 * in the nmap fingerprint file
 */
...
personality_config_new(const char *name, int lineno)
{
	...
	/* personality_new return NULL if a personality does already exists */
...
personality_parse(FILE *fin)
{
	...
	syslog(LOG_DEBUG,"personality parse");
	...
	while ((p = fgets(line, sizeof(line), fin)) != NULL) {
		...
		if (CMP(p, FINGERPRINT) == 0) {
			...
			/* check if fingerprint has a proper name */
			if (!isalnum(*p)) {
				fprintf(stderr, "%d: bad name \"%s\"\n",
				    lineno, p);
				return (-1);
			}
			/* remove the blanks at the end of the line */
			for (p2 = p + strlen(p) - 1; isblank(*p2); p2--)
				*p2 = '\0';

			/* 
			 * add a new config, p contains the name of the fingerprint,
			 * personality_config_new just initializes a personality and
			 * links it to the corresponding xprobe fingerprint if possible,
			 * the actual parsing is done by personality_line below
			 */
