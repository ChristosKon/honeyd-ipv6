...
%token  OBRACKET CBRACKET RBRACKET LBRACKET
...
%token  RANDOMIPVS RANDOMEXCLUDE
%token  SUBMISSION
%token	<string> STRING
%token	<string> CMDSTRING
%token	<string> IPSTRING
%token 	<string> IPSSTRING
%token  <string> FILENAMESTRING
%token  <string> YESNO
%token	<number> NUMBER
...
config		: /* empty */
		...
		| config option
		| config ui
		| config randomipv6mode
		| config randomexclude
		;

randomipv6mode	: RANDOMIPVS FLOAT template LONG
	{
		enable_ipv6_random_mode($2,$4);
		char *default_template = RANDOM_IPV6_DEFAULT_TEMPLATE;
		if (template_clone(default_template, $3, NULL, 1) == NULL) {
			break;
		}	
	}
;

randomexclude	: RANDOMEXCLUDE ip6addr
	{
		exclude_addr_from_generator(addr_ntoa(&$2));
	}
;
...
binding		: BIND ipaddr template
		...
		| BIND ip6addr template
	{
		...
		/* add the template */
		if (template_clone(addr_ntoa(&$2), $3, NULL, 1) == NULL) {
			yyerror("Binding of ipv6 address  to %s failed", addr_ntoa(&$2));
			break;
		}	
...
set		: SET template DEFAULT PROTO ACTION action
		...
		| SET template PERSONALITY personality
	{
		if ($2 == NULL || $4 == NULL)
			break;
		if($4->person4 != NULL)
			$2->person = personality_clone($4->person4);
		if($4->person6 != NULL)
			$2->person6 = personality_clone($4->person6);	
...
*/               | ROUTE ENTRY ip6addr NETWORK ip6net
        {
		syslog(LOG_DEBUG,"ipv6 entry router with %s for address space %s found",addr_ntoa(&$3),addr_ntoa(&$5));
		if (router_start(&$3, &$5) == -1)
			yyerror("Defining entry point failed: %s",
			    addr_ntoa(&$3));
        }
                | ROUTE ip6addr ADD NET ip6net ip6addr latency packetloss bandwidth randomearlydrop
        {
		struct router *r, *newr;
		struct addr defroute;
...
ip6addr		: IPSSTRING
	{
		if(addr_pton($1,&$$)<0)
			yyerror("Illegal IPv6 address %s",$1);	
		free($1);
	}
;
...
ip6net		: ip6addr SLASH NUMBER
	{
		...
		addr_net(&$$, &b);
		b.addr_bits = $$.addr_bits;
		if (memcmp(&$$.addr_ip6, &b.addr_ip6, IP6_ADDR_LEN)) {
			$$ = b;
			yywarn("Bad network mask in %s", src);
		}
	}
...
ipaddrplusport	: ipaddr COLON NUMBER
	{
		...
		| LBRACKET ip6addr RBRACKET COLON NUMBER
	{
		if (curtype == -1) {
			yyerror("Bad port type");
			break;
		}
		$$ = cmd_proxy_getinfo(addr_ntoa(&$2), curtype, $5);
		curtype = -1;
		if ($$ == NULL)
...
personality	: CMDSTRING
	{
		...
		person_set->person4 = personality_find($1+1);
		person_set->person6 = personality_find6($1+1);
		$$ = person_set;
		if ($$->person4 == NULL && $$->person6 == NULL)
