...
struct template
{
	...
	TAILQ_HEAD(subsyscontainerqueue, subsystem_container)
	subsystems;

	/* Condition on which this template is activated */
	TAILQ_HEAD(conditionqueue, condition)
	dynamic;
	int dynamic_rulenr;
...
void template_subsystem_start(struct template *tmpl, struct subsystem *sub);
...
struct template *template_find_best(const char *, const struct ip_hdr *,
		u_short);
void template_list_glob(struct evbuffer *buffer, const char *pattern);
...
void template_subsystem_list_glob(struct evbuffer *buffer, const char *pattern);
...
int template_iterate(int (*f)(struct template *, void *), void *arg);
