...
proxy_logline(struct proxy_ta *ta)
{
	...
	{
		snprintf(line, sizeof(line), "%d %s: CONNECT %s:%s", time(NULL ),
				srcipaddress, host, port);
	}
	else
	{
		snprintf(line, sizeof(line), "%d %s: GET %s:%s%s", time(NULL ),
				srcipaddress, host, port, uri);
	}

	return (line);
}

void proxy_clear_state(struct proxy_ta *ta)
{
	/* XXX - something here */
}
...
int proxy_allowed_network(const char *host)
{
	const char *error;
	int erroroffset;
	pcre *re_uri;
	...
	"^172\\.(1[6-9]|2[0-9]|3[01])\\.[0-9]+\\.[0-9]+$",
			...
			"^0\\.[0-9]+\\.[0-9]+\\.[0-9]+$",
			"^255\\.[0-9]+\\.[0-9]+\\.[0-9]+$", NULL };

	char **p;

	for (p = &unusednets[0]; *p; ++p)
	{
		re_uri = pcre_compile(*p, PCRE_CASELESS, &error, &erroroffset, NULL );
		if (re_uri == NULL )
		{
			/* Default to no match */
			fprintf(stderr, "%s: %s: %s at %d", __func__, *p, error,
					erroroffset);
			return (0);
		}

		/* Match against the URI */
		rc = pcre_exec(re_uri, NULL, host, strlen(host), 0, 0, ovector, 30);
		pcre_free(re_uri);
...
int proxy_allowed_get(struct proxy_ta *ta, struct keyvalue data[])
{
	const char *error;
	int erroroffset;
	char *host, *uri;
	...
	re_uri = pcre_compile(cur->value, PCRE_CASELESS, &error, &erroroffset,
			NULL );
	if (re_uri == NULL )
	{
		/* Default to no match */
		fprintf(stderr, "%s: %s: %s at %d", __func__, cur->value, error,
				erroroffset);
		return (0);
	}
...
int proxy_bad_connection(struct proxy_ta *ta)
{
...
void proxy_remote_readcb(struct bufferevent *bev, void *arg)
{
...
void proxy_remote_writecb(struct bufferevent *bev, void *arg)
{
}

void proxy_remote_errorcb(struct bufferevent *bev, short what, void *arg)
{
	struct proxy_ta *ta = arg;
...
void proxy_connect_cb(int fd, short what, void *arg)
{
	char line[1024], *data;
	struct proxy_ta *ta = arg;
	int error;
	...
	ta->remote_bev = bufferevent_new(ta->remote_fd, proxy_remote_readcb,
	...
	while ((data = kv_find(&ta->dictionary, "data")) != NULL )
	{
		...
		if (strncasecmp(X_FORWARDED, data, strlen(X_FORWARDED)))
		{
			bufferevent_write(ta->remote_bev,
					ta->corrupt ? proxy_corrupt(data, strlen(data)) : data,
					strlen(data));
			bufferevent_write(ta->remote_bev, "\r\n", 2);
		}
...
void proxy_connect(struct proxy_ta *ta, char *host, int port)
{
	...
	if (proxy_allowed_network(host))
	{
		...
		if (local_ip != NULL )
		{
			ta->remote_fd = make_bound_connect(SOCK_STREAM, host, port,
					local_ip);
		}
		else
		{
			ta->remote_fd = make_socket(connect, SOCK_STREAM, host, port);
		}
	}
	if (ta->remote_fd == -1)
	{
...
void proxy_handle_get_cb(int result, char type, int count, int ttl,
...
int proxy_handle_get(struct proxy_ta *ta)
{
	...
	rc = pcre_exec(re_hostport, NULL, host, strlen(host), 0, 0, ovector, 30);
	if (rc >= 0)
	{
...
void proxy_handle_connect_cb(int result, char type, int count, int ttl,
		void *addresses, void *arg)
{
	struct proxy_ta *ta = arg;
	char *host = kv_find(&ta->dictionary, "$host");
	...
	else
	{
		...
		if (fd != -1)
			smtp_ta = smtp_ta_new(fd, (struct sockaddr *) &ta->sa, ta->salen,
					NULL, 0, 0);
		if (smtp_ta != NULL )
		{
			response = proxy_response(ta, goodport);
			bufferevent_write(smtp_ta->bev, response, strlen(response));
...
int proxy_handle_connect(struct proxy_ta *ta)
{
	char *host = kv_find(&ta->dictionary, "$rawhost");
	int rc;
	int ovector[30];
...
int proxy_handle(struct proxy_ta *ta, char *line)
{
	int rc;
	int ovector[30];
	...
	rc = pcre_exec(re_connect, NULL, line, strlen(line), 0, 0, ovector, 30);
	if (rc >= 0)
	{
...
void proxy_errorcb(struct bufferevent *bev, short what, void *arg)
{
	fprintf(stderr, "%s: called with %p, freeing\n", __func__, arg);
...
struct proxy_ta *
proxy_ta_new(int fd, struct sockaddr *sa, socklen_t salen, struct sockaddr *lsa,
		socklen_t lsalen)
{
	...
	ta->fd = fd;
	ta->bev = bufferevent_new(fd, proxy_readcb, proxy_writecb, proxy_errorcb,
			ta);
	if (ta->bev == NULL )
		goto error;
...
static void accept_socket(int fd, short what, void *arg)
{
	...
	{
		proxy_ta_new(nfd, (struct sockaddr *) &ss, addrlen, NULL, 0);
	}
}
...
void proxy_init(void)
{
	...
	re_connect = pcre_compile(exp_connect, PCRE_CASELESS, &error, &erroroffset,
			NULL );
	if (re_connect == NULL )
		err(1, "%s: %s at %d", __func__, error, erroroffset);

	re_hostport = pcre_compile(exp_hostport, PCRE_CASELESS, &error,
			&erroroffset, NULL );
	if (re_connect == NULL )
		err(1, "%s: %s at %d", __func__, error, erroroffset);

	re_get = pcre_compile(exp_get, PCRE_CASELESS, &error, &erroroffset, NULL );
	if (re_connect == NULL )
		err(1, "%s: %s at %d", __func__, error, erroroffset);
}
