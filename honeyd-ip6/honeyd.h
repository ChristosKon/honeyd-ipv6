...
#define HONEYD_MAX_CONNECTS	5 /*32000*/
...
{ /* dnet.h define this but the size is wrong */
	/* So define ours */
...
int cmd_subsystem_connect(struct tuple *hdr, struct command *cmd, struct port *,
		void *arg);
...
void icmp6_send_pkt(const struct interface *, struct addr *, struct addr *,
		struct in6_addr *, struct in6_addr *, u_char *, int);
void
honeyd_deliver_ethernet6(const struct interface *, struct addr *, struct addr *,
		struct ip6_hdr *, u_int);
//void icmp6_send_neighbor_adv(struct interface *,struct addr *,struct addr *,struct in6_addr *,struct in6_addr *);
