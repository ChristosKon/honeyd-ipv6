...
struct subsystem
{
	...
	SPLAY_HEAD(subtmpltree, template_container)
	root;
	/* all templates */
	TAILQ_HEAD(templateq, template_container)
	templates;
	char *cmdstring;
...
#define SUBSYSTEM_RESTART	0x02
	...
	TAILQ_HEAD(portqueue, port)
	ports; /* list of configured ports */
};
...
int templ_container_compare(struct template_container *,
		struct template_container *);
SPLAY_PROTOTYPE( subtmpltree, template_container, node, templ_container_compare);
