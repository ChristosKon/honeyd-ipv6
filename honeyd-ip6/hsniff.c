...
void usage(void)
{
	fprintf(stderr, "Usage: hsniff [OPTIONS] [net ...]\n\n"
...
void hsniff_settcp(struct tcp_track *con, struct ip_hdr *ip,
...
void hsniff_setudp(struct udp_con *con, struct ip_hdr *ip, struct udp_hdr *udp,
		int local)
{
	struct tuple *hdr = &con->conhdr;
...
void hsniff_init(void)
{
	/* Initalize ongoing connection state */
	SPLAY_INIT(&tcpcons);
}

void hsniff_exit(int status)
{
	interface_close_all();
...
void tcp_track_free(struct tcp_track *con)
{
...
void hsniff_tcp_timeout(int fd, short event, void *arg)
{
	struct tcp_track *con = arg;

	syslog(LOG_DEBUG, "Expiring TCP %s (%p)", honeyd_contoa(&con->conhdr), con);

	tcp_track_free(con);
}

void tcp_insert(struct tcp_track *con, uint32_t th_seq, void *data, size_t dlen)
{
...
void tcp_drop_subsumed(struct tcp_track *con)
{
	struct tcp_segment *seg;
	uint32_t off;
	...
	while ((seg = TAILQ_FIRST(&con->segments))!= NULL)
	{
		...
		syslog(LOG_NOTICE, "Streaming: %s %u: %d", honeyd_contoa(&con->conhdr),
				con->snd_una, seg->len);
...
void tcp_recv_cb(u_char *pkt, u_short pktlen)
{
	struct ip_hdr *ip;
	struct tcp_hdr *tcp;
	...
	if (th_seq == con->snd_una)
	{
		...
		syslog(LOG_NOTICE, "Streaming: %s %u: %d", honeyd_contoa(&con->conhdr),
				con->snd_una, dlen);
		hooks_dispatch(IP_PROTO_TCP, HD_INCOMING_STREAM, &con->conhdr, data,
				dlen);
...
void udp_recv_cb(u_char *pkt, u_short pktlen)
{
...
void hsniff_recv_cb(u_char *ag, const struct pcap_pkthdr *pkthdr,
		const u_char *pkt)
{
...
void hsniff_signal(int fd, short what, void *arg)
{
...
int main(int argc, char *argv[])
{
	...
	orig_argc = argc;
	orig_argv = argv;
	while ((c = getopt_long(argc, argv, "VPUdc:i:u:g:f:0:h?", hsniff_long_opts,
			NULL )) != -1)
	{
		char *ep;
		switch (c)
	...
	while (argc > 0)
	{
		...
		{
			snprintf(line, sizeof(line), "(not src net %s and dst net %s)",
					addr_ntoa(&addr), addr_ntoa(&addr));
			if (strlcat(filter, line, sizeof(filter)) >= sizeof(filter))
				errx(1, "too many address; filter too long");
		}
	...
	stats_init();
	stats_init_collect(&stats_dst, stats_port, stats_username, stats_password);
...
void droppriv(uid_t uid, gid_t gid)
{
	...
	if (setegid(gid) == -1)
		SETERROR(
				(errline, sizeof(errline), "%s: setegid(%d) failed\n", __func__, gid));
	if (setgid(gid) == -1)
		SETERROR(
				(errline, sizeof(errline), "%s: setgid(%d) failed\n", __func__, gid));
#ifdef HAVE_SETREUID
	...
	if (setuid(uid) == -1)
		SETERROR(
				(errline, sizeof(errline), "%s: setuid(%d) failed\n", __func__, gid));

	if (getgid() != gid || getegid() != gid)
	{
		SETERROR(
				(errline, sizeof(errline), "%s: could not set gid to %d", __func__, gid));
		goto error;
	}
	...
	{
		SETERROR(
				(errline, sizeof(errline), "%s: could not set uid to %d", __func__, uid));
		goto error;
	}
	...
	{
		SETERROR(
				(errline, sizeof(errline), "%s: did not successfully drop privilege", __func__));
		goto error;
	}
	...
	return;
	error: syslog(LOG_WARNING, error);
