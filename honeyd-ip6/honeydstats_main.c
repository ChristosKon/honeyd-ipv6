...
static void read_cb(int fd, short what, void *arg)
{
	...
	syslog(LOG_INFO, "Received report from %s: %d", addr_ntoa(&src), nread);
...
void usage(void)
{
	fprintf(stderr, "Usage: honeydstats [OPTIONS]\n\n"
...
void setup_socket(char *address, int port)
{
...
void honeydstats_signal(int fd, short what, void *arg)
{
	syslog(LOG_NOTICE, "exiting on signal %d", fd);
	exit(0);
}

void honeydstats_sighup(int fd, short what, void *arg)
{
	syslog(LOG_NOTICE, "rereading configuration/rotating files on signal %d",
			fd);
	...
	{
		close(checkpoint_fd);
		checkpoint_fd = open(checkpoint_filename, O_CREAT | O_WRONLY | O_APPEND,
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "TVdc:r:l:p:f:h?", stats_long_opts,
			NULL )) != -1)
	{
		switch (c)
		{
