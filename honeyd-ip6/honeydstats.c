...
static int user_compare(struct honeyuser *a, struct honeyuser *b)
{
...
SPLAY_PROTOTYPE(usertree, honeyuser, node, user_compare)
;
SPLAY_GENERATE(usertree, honeyuser, node, user_compare)
;
...
void user_new(const char *name, const char *password)
{
	struct honeyuser *user = NULL, tmp;

	tmp.name = name;
	if ((user = SPLAY_FIND(usertree, &users, &tmp)) == NULL )
	{
		if ((user = calloc(1, sizeof(struct honeyuser))) == NULL )
...
int user_read_config(const char *filename)
{
	...
	while (fgets(line, sizeof(line), fin) != NULL )
	{
		...
		{
			syslog(LOG_WARNING, "%s:%d: cannot read user and password",
					filename, nrline);
			goto out;
		}
...
int record_process(struct honeyuser *user, struct evbuffer *evbuf)
{
	...
	{
		syslog(LOG_WARNING, "%s: failed to unmashal record for user '%s'",
				__func__, user->name);
		goto out;
	}
	...
	res = 0;
	out: record_clean(record);
	free(record);
	return (res);
}

int measurement_process(struct honeyuser *user, struct evbuffer *evbuf)
{
	uint32_t counter;
	...
	if (!checkpoint_doreplay || user->nreports % 60 == 0)
		syslog(LOG_INFO, "%s: %ld seconds of data at measurement period %.24s",
	...
	{
		syslog(LOG_WARNING, "%s: replayed packet: %d, expecting %d", user->name,
				counter, user->seqnr);
		return (-1);
	}
	...
	else if (checkpoint_doreplay && timercmp(&checkpoint_tv, &tv_end, <))
	{
...
{
	struct honeyuser *user = NULL, tmpuser;
	uint8_t tag;
	...
	if (checkpoint_fd != -1)
	{
		...
		evbuffer_add(checkpoint_evbuf, EVBUFFER_DATA(evbuf),
				EVBUFFER_LENGTH(evbuf));
	}

	if (tag_unmarshal_string(evbuf, SIG_NAME, &username) == -1)
		goto out;
	if (tag_unmarshal_fixed(evbuf, SIG_DIGEST, digest, sizeof(digest)) == -1)
		goto out;
	...
	/* Validate signature */
	if (!hmac_verify(&user->hmac, digest, sizeof(digest), EVBUFFER_DATA(tmp),
			EVBUFFER_LENGTH(tmp)))
	{
	...
	case SIG_DATA:
		...
		break;
	default:
		syslog(LOG_NOTICE, "%s: unknown signature tag %d", __func__, tag);
		goto out;
	}
...
static int signature_length(struct evbuffer *evbuf)
{
	struct evbuffer tmp;
